...
console.log('🔧 Comprehensive JSX Fixer...\n');

// Create backup
const backupPath = './src/app/project-manager/page.tsx.backup18';
fs.writeFileSync(backupPath, content);
console.log('📄 Created backup at:', backupPath);

// Fix the JSX structure systematically
function fixJSXStructure(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Fix the specific problematic area around line 1511
    if (i === 1510 && trimmedLine === ')}') {
      // This is the problematic line, replace it with proper structure
      fixedLines.push('                        </div>');
      fixedLines.push('                      </div>');
      fixedLines.push('                </div>');
      fixedLines.push('              ) : (');
    ...
    // Fix the end of the file structure
    if (i === 1563 && trimmedLine === ')}') {
      // Add missing closing brace for main conditional
      fixedLines.push('          )}');
      fixedLines.push('        </main>');
      continue;
    }
    
    // Remove extra div on line 1565
    if (i === 1564 && trimmedLine === '                  </div>') {
      // Skip this line as it's extra
      continue;
    }
...
// Apply the fix
console.log('🔧 Applying comprehensive JSX fix...');
let fixedContent = fixJSXStructure(content);

// Write the fixed content
fs.writeFileSync(filePath, fixedContent);

console.log('✅ Applied comprehensive JSX fix');
