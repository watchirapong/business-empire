...
async function resetGachaItems() {
  try {
    await connectDB();

    // Clear existing items
    const deleteResult = await GachaItem.deleteMany({});
    console.log(`Deleted ${deleteResult.deletedCount} existing gacha items`);

    // Insert new items
    const insertedItems = await GachaItem.insertMany(defaultGachaItems);
    console.log(`Successfully added ${insertedItems.length} new gacha items!`);
    ...
    const totalDropRate = insertedItems.reduce((sum, item) => sum + item.dropRate, 0);
    console.log(`\nTotal drop rate: ${totalDropRate}%`);

  } catch (error) {
    console.error('Error resetting gacha items:', error);
  } finally {
